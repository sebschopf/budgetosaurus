...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showToast(message, tags, duration = 5000) {
        ...
        // Les tags Django peuvent contenir plusieurs classes (ex: 'success autres_tags')
        ...
        for (const tag of messageTags) {
            if (tagMap[tag]) {
                ...
                foundTag = true;
                break; // Utilise le premier tag reconnu
            }
        }
        if (!foundTag) {
            // Fallback pour les tags non reconnus ou 'info' par défaut
        ...
        closeButton.addEventListener('click', function() {
            clearTimeout(hideTimeout); // Empêche le fadeOut automatique si fermé manuellement
            dismissToast(toast);
        });
        ...
        toast.addEventListener('mouseover', () => {
            ...
            toast.style.animationPlayState = 'paused'; // Met en pause l'animation fadeOut
        });

        // Reprendre le fadeOut si la souris quitte le toast
        toast.addEventListener('mouseleave', () => {
            toast.style.animationPlayState = 'running'; // Reprend l'animation fadeOut
            hideTimeout = setTimeout(() => {
                dismissToast(toast);
            }, 1000); // Donne un court délai avant de masquer après mouseleave
        });
    }
    ...
    function dismissToast(toastElement) {
        ...
        toastElement.addEventListener('animationend', () => {
            toastElement.remove();
        }, { once: true }); // S'assure que l'écouteur n'est déclenché qu'une seule fois
    }
});
